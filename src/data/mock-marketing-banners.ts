import type { MarketingBanner } from '@/types/marketing-banner';

export const marketingBanners: MarketingBanner[] = [
  {
    id: 'banner-1',
    title: 'Шинэ үеийн өндөр харагдацтай иж бүрдэл',
    titleEn: 'Premium Site Visibility Kits',
    titleMn: 'Шинэ үеийн өндөр харагдацтай иж бүрдэл',
    slug: 'premium-site-visibility-kits',
    summary: 'Уурхай, барилга, үйлдвэрийн талбайд ажиллах багуудад зориулсан өндөр харагдацтай шинэ иж бүрдэл.',
    summaryEn: 'A new high-visibility equipment bundle built for teams working across mining, construction, and industrial sites.',
    summaryMn: 'Уурхай, барилга, үйлдвэрийн талбайд ажиллах багуудад зориулсан өндөр харагдацтай шинэ иж бүрдэл.',
    content:
      'FrogWard нь гэрэлтүүлэг багатай болон хөдөлгөөн ихтэй ажлын орчинд зориулагдсан өндөр харагдацтай шинэ иж бүрдлийг санал болгож байна.\n\nЭнэхүү иж бүрдэл нь өндөр харагдацтай гадуур хувцас, удаан эдэлгээтэй ойлгогч тууз, үйлдвэрийн орчинд давтан ашиглахад тохирсон материалуудыг хослуулсан.\n\nБагийн худалдан авалтыг хурдан шийдэх шаардлагатай байгууллага, гүйцэтгэгч нарт зориулсан энэхүү цуврал нь аюулгүй байдал, тав тухыг зэрэг хангах зорилготой.',
    contentEn:
      'FrogWard is introducing a new high-visibility equipment bundle built for low-light and high-traffic worksites.\n\nThe bundle combines high-visibility outerwear, durable reflective trims, and materials suited for repeated industrial use.\n\nIt is aimed at organizations and contractors that need to equip teams quickly without sacrificing safety or comfort.',
    contentMn:
      'FrogWard нь гэрэлтүүлэг багатай болон хөдөлгөөн ихтэй ажлын орчинд зориулагдсан өндөр харагдацтай шинэ иж бүрдлийг санал болгож байна.\n\nЭнэхүү иж бүрдэл нь өндөр харагдацтай гадуур хувцас, удаан эдэлгээтэй ойлгогч тууз, үйлдвэрийн орчинд давтан ашиглахад тохирсон материалуудыг хослуулсан.\n\nБагийн худалдан авалтыг хурдан шийдэх шаардлагатай байгууллага, гүйцэтгэгч нарт зориулсан энэхүү цуврал нь аюулгүй байдал, тав тухыг зэрэг хангах зорилготой.',
    image: 'https://images.unsplash.com/photo-1517048676732-d65bc937f952?w=1200&h=800&fit=crop',
    focalX: 50,
    focalY: 50,
    zoom: 100,
    active: true,
  },
  {
    id: 'banner-2',
    title: 'Урт ээлжийн ажилд зориулсан хамгаалалтын гутал',
    titleEn: 'Boots Built for Long Shifts',
    titleMn: 'Урт ээлжийн ажилд зориулсан хамгаалалтын гутал',
    slug: 'boots-built-for-long-shifts',
    summary: 'Ус нэвтрэхгүй, төмөр хоншоортой, хүйтэнд тэсвэртэй хамгаалалтын гутлын шинэ загвар.',
    summaryEn: 'New waterproof, steel-toe, cold-resistant safety boots designed for demanding long-shift work.',
    summaryMn: 'Ус нэвтрэхгүй, төмөр хоншоортой, хүйтэнд тэсвэртэй хамгаалалтын гутлын шинэ загвар.',
    content:
      'Энэхүү шинэ гутлын цуврал нь үйлдвэрийн урт ээлжийн ажилд зориулан бүтээгдсэн.\n\nУс нэвтрэхгүй хийц, бэхэлсэн хоншоорын хамгаалалт, хүйтэн нөхцөлд ажиллах боломжийг нэг дор шингээсэн тул уурхай, логистик, хүнд барилгын ажлын орчинд тохиромжтой.\n\nӨдөржин өмсөхөд эвтэйхэн атлаа элэгдэл даах чадвартай шийдэл хэрэгтэй хэрэглэгчдэд зориулав.',
    contentEn:
      'This new boot range is designed for long industrial shifts.\n\nIts waterproof construction, reinforced toe protection, and cold-weather readiness make it suitable for mining, logistics, and heavy construction environments.\n\nIt is built for customers who need comfort through the day without giving up durability.',
    contentMn:
      'Энэхүү шинэ гутлын цуврал нь үйлдвэрийн урт ээлжийн ажилд зориулан бүтээгдсэн.\n\nУс нэвтрэхгүй хийц, бэхэлсэн хоншоорын хамгаалалт, хүйтэн нөхцөлд ажиллах боломжийг нэг дор шингээсэн тул уурхай, логистик, хүнд барилгын ажлын орчинд тохиромжтой.\n\nӨдөржин өмсөхөд эвтэйхэн атлаа элэгдэл даах чадвартай шийдэл хэрэгтэй хэрэглэгчдэд зориулав.',
    image: 'https://images.unsplash.com/photo-1525966222134-fcfa99b8ae77?w=1200&h=800&fit=crop',
    focalX: 50,
    focalY: 50,
    zoom: 100,
    active: true,
  },
  {
    id: 'banner-3',
    title: 'Байгууллагын хэмжээнд багийг бүрэн хангах шийдэл',
    titleEn: 'Outfit Teams at Scale',
    titleMn: 'Байгууллагын хэмжээнд багийг бүрэн хангах шийдэл',
    slug: 'outfit-teams-at-scale',
    summary: 'ХАБЭА хэрэгслийн улирлын нөөцлөлт болон багц худалдан авалтад зориулсан шуурхай нийлүүлэлт.',
    summaryEn: 'Fast supply planning for seasonal PPE restocking and large team purchasing.',
    summaryMn: 'ХАБЭА хэрэгслийн улирлын нөөцлөлт болон багц худалдан авалтад зориулсан шуурхай нийлүүлэлт.',
    content:
      'FrogWard нь олон хэлтэс, олон төслийн хэмжээнд тогтмол хамгаалах хэрэгсэл шаарддаг байгууллагуудад зориулсан нийлүүлэлтийн шийдлийг санал болгодог.\n\nЭнэхүү санал нь улирлын нөөц бүрдүүлэлт, давтан захиалга, бүх багийг хамарсан худалдан авалтыг илүү хурдан бөгөөд цэгцтэй зохион байгуулахад чиглэнэ.\n\nҮйлдвэрлэл, барилга, уул уурхайд тохирох бүтээгдэхүүний сонголттойгоор байгууллагын хэрэгцээнд илүү ойр ажиллах зорилготой.',
    contentEn:
      'FrogWard offers supply support for organizations that regularly need PPE across multiple departments and projects.\n\nThe approach is designed to make seasonal restocking, repeat orders, and large team procurement faster and more structured.\n\nIt is intended to support manufacturing, construction, and mining customers with selections that fit operational needs more closely.',
    contentMn:
      'FrogWard нь олон хэлтэс, олон төслийн хэмжээнд тогтмол хамгаалах хэрэгсэл шаарддаг байгууллагуудад зориулсан нийлүүлэлтийн шийдлийг санал болгодог.\n\nЭнэхүү санал нь улирлын нөөц бүрдүүлэлт, давтан захиалга, бүх багийг хамарсан худалдан авалтыг илүү хурдан бөгөөд цэгцтэй зохион байгуулахад чиглэнэ.\n\nҮйлдвэрлэл, барилга, уул уурхайд тохирох бүтээгдэхүүний сонголттойгоор байгууллагын хэрэгцээнд илүү ойр ажиллах зорилготой.',
    image: 'https://images.unsplash.com/photo-1581092580497-e0d23cbdf1dc?w=1200&h=800&fit=crop',
    focalX: 50,
    focalY: 50,
    zoom: 100,
    active: true,
  },
];
