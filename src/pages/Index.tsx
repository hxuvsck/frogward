    ...
    <Layout>
        ...
        <div className="container relative z-10">
          <div className="max-w-xl space-y-6">
            <div className="inline-block rounded-full border border-primary/30 bg-primary/10 px-4 py-1.5">
              <span className="text-xs font-medium text-primary">Industrial Grade Equipment</span>
            </div>
            <h1 className="font-heading text-5xl md:text-6xl font-bold leading-[1.1] tracking-tight">
              Built for the <span className="text-primary">toughest</span> conditions
            </h1>
            <p className="text-lg text-muted-foreground leading-relaxed">
              Professional workwear and PPE equipment trusted by industries across Mongolia.
            </p>
      ...
      <section className="border-y border-border">
        <div className="container grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border">
          {[
            { icon: Shield, title: 'Certified Safety', desc: 'All products meet international safety standards' },
            { icon: Truck, title: 'Fast Delivery', desc: 'Same-day delivery in Ulaanbaatar' },
            { icon: Headphones, title: '24/7 Support', desc: 'Expert support for equipment selection' },
          ].map((f) => (
      ...
      <section className="container py-16">
        <h2 className="font-heading text-2xl font-bold mb-8">Shop by Category</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {categories.map((cat) => (
            ...
            >
              <h3 className="font-heading text-sm font-semibold group-hover:text-primary transition-colors">{cat.name}</h3>
